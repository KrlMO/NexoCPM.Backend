...
namespace NexoCPM.Application.Curriculum.Ports
{
    public interface ISyllabusUnitRepository
    {
        ...
        Task<List<UserSyllabusTopicData>> GetUnitTopicsAsync(int syllabusUnitId, int userSyllabusProgressId);
        Task<List<UserSyllabusSubtopicData>> GetTopicSubtopicsAsync(int topicId, int userSyllabusProgressId);
    }
}
